...
<%@ page import="com.maro.common.util.Util" %>
<%@ page import="com.maro.platform.core.util.ApplicationContextUtil" %>
<%@ page import="com.maro.platform.web.system.service.SystemService" %>
...
<%
	...
	List<Object> queueNames = systemService.findListbySql("select  t.org_code from t_s_depart t where t.or_not_store='Y'");
	int size=queueNames.size();
	String queueName="";
...
<div class="layout-admin">
	...
	<header class="layout-header">
		<span class="header-logo">子煊智慧餐饮管理系统</span>
		...
		<ul class="header-bar">
			...
			<li class="header-bar-nav personInfo" style="cursor:pointer;">
				...
				<ul class="header-dropdown-menu" style="padding-right:4px">
					...
					<c:if test="<%=admin%>">
						<li >
							<a href="javascript:syncData()" title="数据同步">数据同步</a>
						</li>
					</c:if>
...
<script type="text/javascript">
    function syncData(){
        ...
        $.dialog.confirm('你确定要同步数据吗?', function(r) {
            if (r) {
                $.ajax({
