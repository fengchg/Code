...
import com.maro.common.print.api.content.printable.NolineTable;
 ...
 * 表格转换器
 */
public class NolineTableConverter extends Converter<NolineTable> {
	...
	public byte[] toBytes(DeviceSetting setting) throws PrinterException {
		...
		// 计算制表位置
		byte[] hPoint = checkCellWidth(columnWidths, StringUtil.getCharEachLine(setting));
		...
		// 设置表体字体
		Font textStyle = new Font(FontSize.Small, false, false);
		out.write(getConverterKit().fromConverter(textStyle, setting));
		// 设置制表位置
		out.write(ESCPOSCode.SET_TABLE_CELL_POSITION(hPoint));
		...
		out.write(getConverterKit().fromConverter(head_line, setting));// 画一条黑虚线
	...
	private byte[] creatBodyBytes(List<Row> rows, List<Integer> columnWidths, List<Boolean> alignRights, DeviceSetting setting) throws PrinterException {
		BytesBuffer out = new BytesBuffer();
		for (Row a_row : rows) {
			if (a_row.equals(NolineTable.LINE_ROW)) {
	 ...
	 * @param charEachLine
	 */
	private byte[] checkCellWidth(List<Integer> columnWidths, int charEachLine) {
		int whole_length = 0;
		byte[] h_position = new byte[columnWidths.size()];
