        ...
        ok: function(){
            ...
            $.each( $(th).parent().parent().parent().find("input[class='inputxt']"), function(i,val){
                if(saveState[1]==$(this).val()){
                    var specificationsId=$(this).parent().next().find("input[type='hidden']").val();
                    if(saveState[2]==specificationsId){
                        isTF = true;
                        return false;
                    }
                }
            ...
            $(th).parent().next().find("input[type='text']").val(saveState[3]);
            $(th).parent().next().find("input[type='hidden']").val(saveState[2]);
            return true;
        },
        cancelVal: '关闭',
