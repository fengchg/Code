...
import com.maro.platform.core.extend.swftools.SwfToolsUtil;
import com.maro.platform.core.extend.template.DataSourceMap;
...
import com.maro.platform.web.system.pojo.base.*;
...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
	...
	public List<TreeGrid> treegrid(List all, TreeGridModel treeGridModel) {
		...
		for (Object obj : all) {
            ...
            //主要是用户组织机构时显示 店铺管理 按钮
            try{
            	//唯一标识，  有 orNotStore 执行
            	String orNotStore = treeGridModel.getFieldMap().get("orNotStore").toString();
            	
                String note = TagUtil.fieldNametoValues(orNotStore, obj).toString();
                tg.setNote(note);
