...
import com.maro.platform.core.common.model.json.ComboBox;
import com.maro.platform.core.common.model.json.DataGrid;
import com.maro.platform.core.common.model.json.ValidForm;
import com.maro.platform.core.constant.Globals;
import com.maro.platform.core.enums.SysThemesEnum;
...
import com.maro.platform.core.util.ListtoMenu;
import com.maro.platform.core.util.MyBeanUtils;
import com.maro.platform.core.util.PasswordUtil;
import com.maro.platform.core.util.ResourceUtil;
import com.maro.platform.core.util.RoletoJson;
import com.maro.platform.core.util.SetListSort;
import com.maro.platform.core.util.StringUtil;
import com.maro.platform.core.util.SysThemesUtil;
...
import com.maro.platform.tag.vo.datatable.DataTableReturn;
import com.maro.platform.tag.vo.datatable.DataTables;
import com.maro.platform.web.system.manager.ClientManager;
import com.maro.platform.web.system.pojo.base.InterroleEntity;
import com.maro.platform.web.system.pojo.base.InterroleUserEntity;
import com.maro.platform.web.system.pojo.base.TSDepart;
import com.maro.platform.web.system.pojo.base.TSFunction;
import com.maro.platform.web.system.pojo.base.TSRole;
import com.maro.platform.web.system.pojo.base.TSRoleFunction;
import com.maro.platform.web.system.pojo.base.TSRoleUser;
import com.maro.platform.web.system.pojo.base.TSUser;
import com.maro.platform.web.system.pojo.base.TSUserOrg;
...
public class UserController extends BaseController {
	...
	@RequestMapping(params = "checkEmpNo")
	@ResponseBody
	public AjaxJson checkEmpNo(HttpServletRequest request,String empNo,String orgIds) {
		AjaxJson j = new AjaxJson();
		TSUser tsUsrt = ResourceUtil.getSessionUser();
		try {
			String org_id = "";
			...
			if(orgIds!=null && !orgIds.equals("")){
				...
				org_id =  tsUsrt.getCurrentDepart().getId();
			}
			String sql = "select count(0) from t_s_user tsu,t_s_user_org tsuo where tsu.id = tsuo.user_id and tsuo.org_id = '"+ org_id +"' and tsu.emp_no = '"+empNo+"'";
			List<Integer> count = systemService.findListbySql(sql);
			j.setObj(count);
		} catch (Exception e) {
			j.setObj(-1);
			e.printStackTrace();
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        //查询条件组装器
        com.maro.platform.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, user);
		...
		for (int i = 0; i < roleCodeArr.length; i++) {
			if (roleCodeArr[i].equals("admin")) {
				tf = 1;
				break;
			} else if (roleCodeArr[i].equals("maro_admin")) {
				tf = 2;
			}
		}
		...
		if(tf == 1){//为admin管理
			...
			String sql = "select id from t_s_depart where id = '"+ tsUsrt.getCurrentDepart().getId() +"' or parentdepartid = '"+ tsUsrt.getCurrentDepart().getId() +"'";
			List<String> oLists = systemService.findListbySql(sql);
			for (int i=0;i<oLists.size();i++) {
				if(i != (oLists.size() - 1)){
					orgIds = orgIds + oLists.get(i) + ",";
				}else{
					orgIds = orgIds + oLists.get(i) ;
				}
			}
		}else{
			orgIds = tsUsrt.getCurrentDepart().getId();
		}
...
**/	
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			Integer n = maroMqSynchronousService.ifShopIsExist(orgid.split(","));
			if(n != 0){
	...
	public void datagridRole(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		com.maro.platform.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsRole);
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("roleCodes", roleCodes);
...
//			}
	...
	public String exportXls(TSUser tsUser,HttpServletRequest request,HttpServletResponse response
		...
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		com.maro.platform.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsUser, request.getParameterMap());
