new Vue({
	...
	methods: {
		...
		openModal: function() {
            ...
            if(this.groupDiscountObj.id !== '' && typeof(this.groupDiscountObj.id) !== "undefined"){
                this.factAmount6 = this.groupDiscountObj.money
            }else {
                this.factAmount6 = 0
			}
		},
		...
		paySuccess: function(index) {
            //index 1:打单，2:不打单
			var print = false;
			if(index === 1)print = true;
			if(index === 2)print = false;
			...
			if(count == 0 && 0 !== (this.pay.payAmount - this.pay.smallChange)) {
				layerToast("支付金额不能为零");
				return;
			}
            ...
            if(this.groupDiscountObj.id !== ''){
                var groupDiscountJson = {
                    'payType': 6,
                    'payTerminal': 0,
                    'amount': this.groupDiscountObj.money,
                    'payId':this.groupDiscountObj.id,
                    'remark':this.groupDiscountObj.remark
                }
                payArray.push(groupDiscountJson);
            }
        ...
        pickCouponSure: function () {
            if (!this.groupDiscount.id || this.groupDiscountObj.id === this.groupDiscount.id) {
                ...
                this.groupDiscountObj.remark = this.groupDiscount.remark;
            ...
            this.groupDiscountUser = '('+this.groupDiscount.name+')';
            this.clsModal();
        },
