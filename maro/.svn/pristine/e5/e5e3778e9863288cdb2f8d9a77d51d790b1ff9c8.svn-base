            ...
            <yd-flexbox >
              <yd-flexbox-item @click.native="goPackage(item)">
                <div class="ord-list-name" >
                  <span class="ord-list-give" v-if="item.type == 2">赠</span>
                  <span>{{item.foodName}}</span>
                  <span v-if="item.packageType == 1">
                     <!--<yd-icon name="star" size="16px" color="#16AA6B" ></yd-icon>-->
                       <img :src="icon_star" class="ord-list-star" />
                  </span>
...
export default {
  data() {
    return {
      icon_star:require('../../assets/icon/detail_star.jpg'),
      orderList: [],
      disNo: '',
      optionFlag: false,
      beginTime: '',
...
//      if(obj.maroClientFoodOrderVOs){
//        for (let j = 0; j < obj.maroClientFoodOrderVOs.length; j++) {
//          let o = obj.maroClientFoodOrderVOs[j];
//          if(o.price === 0){
//            o.type = 2
//          }
//        }
//      }
      this.$store.dispatch('setFoodOrder', obj.maroClientFoodOrderVOs)
    },
...
<style scoped>
  ...
  .ord-list-star{width: .4rem;position: absolute}
  .buy-list-head {
