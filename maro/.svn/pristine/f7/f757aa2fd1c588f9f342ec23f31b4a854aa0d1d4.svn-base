...
import com.maro.platform.core.util.*;
...
import com.maro.platform.core.enums.StoreUploadFilePathEnum;
import com.maro.platform.core.extend.hqlsearch.parse.ObjectParseUtil;
import com.maro.platform.core.extend.hqlsearch.parse.PageValueConvertRuleEnum;
import com.maro.platform.core.extend.hqlsearch.parse.vo.HqlRuleEnum;
...
import com.maro.platform.web.system.manager.ClientSort;
import com.maro.platform.web.system.pojo.base.Client;
import com.maro.platform.web.system.pojo.base.DataLogDiff;
import com.maro.platform.web.system.pojo.base.TSDatalogEntity;
...
public class SystemController extends BaseController {
	...
	public void typeGroupGrid(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid, TSTypegroup typegroup) {
		...
		if(userName.toLowerCase().equals("admin")||roleCodes.indexOf("admin")!=-1){//管理员
...
//    }
	 ...
	 * @return
	 */
	@RequestMapping(params = "mqRefreshTypeGroupAndTypes")
	@ResponseBody
	public AjaxJson MqRefreshTypeGroupAndTypes() {
		AjaxJson j = new AjaxJson();
		try{
			Thread.sleep(5*1000);
			systemService.refreshTypeGroupAndTypes();
			System.out.println("字典刷新完成！");
		} catch (Exception e) {
			e.printStackTrace();
			j.setSuccess(false);
			j.setMsg("刷新失败！");
...
//			depart.setOrgCode(orgCode);
    ...
    public void dataLogDatagrid(TSDatalogEntity datalogEntity,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid){
		...
		//查询条件组装器
		com.maro.platform.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, datalogEntity, request.getParameterMap());
		cq.add();
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
    }
