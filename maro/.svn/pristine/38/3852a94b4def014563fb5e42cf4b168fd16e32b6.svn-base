package com.maro.client.module.pay.controller;
...
//	private static final String appID="wx230c617d7f6c9677";
//	private static final String secret="ce79ccd023ae70777e848c0ed90864a3";
//	private static final String mchID="1507628441";
//	private static final String partnerKey="wx230c617d7f6c9677CDZX168CDZXZIX";
...
//	}
	...
	public AjaxJson webPay(HttpServletRequest request, HttpServletResponse response) {
		AjaxJson ajaxJson = new AjaxJson();
		System.out.println("------------------gzhPay start-------------------");
		JSONObject jsonObject = new JSONObject();
		String total_fee = request.getParameter("total_fee");
		log.error("进入支付方法，支付"+total_fee);
		//获取用户的code
		...
		WxPayApiConfigKit.putApiConfig(WechatUtil.getApiConfig());
		Map<String, String> params = WxPayApiConfigKit.getWxPayApiConfig()
				                             ...
				                             .setNotifyUrl(WechatUtil.notify_url + "&payId" + payId)
				                             .setOutTradeNo(String.valueOf(System.currentTimeMillis()))
				                             .build();
		String xmlResult = WxPayApi.pushOrder(false,params);
		log.error("pushOrder后的返回结果:"+xmlResult);
		...
		packageParams.put("appId", WechatUtil.appID);
		...
		jsonObject.put("appid",  WechatUtil.appID);
...
//		if (order==null) {
		WxPayApiConfigKit.setThreadLocalWxPayApiConfig(WechatUtil.getApiConfig());
		if(PaymentKit.verifyNotify(params, WxPayApiConfigKit.getWxPayApiConfig().getPaternerKey())){

			if (("SUCCESS").equals(result_code)) {
				//修改foodOrder删除标志
				serverOrderApplyService.updateFoodOrderDeleteFlag(payId,CommonTypeEnum.DELETE_FLAG_NO.getCode());
				//更新订单信息
				log.error("支付成功，更新支付信息:"+payId);
				//发送通知等
