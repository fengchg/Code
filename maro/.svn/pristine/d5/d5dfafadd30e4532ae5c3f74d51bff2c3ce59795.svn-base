...
public class MaroSynchronizationPrinterController extends BaseController {
	...
	public AjaxJson doDel(String message) {
		...
		try{
			maroPrinterService.delMain((MaroPrinterEntity)Util.jsonToObject(message,0));
		}catch(Exception e){
			e.printStackTrace();
	...
	public AjaxJson doAdd(String message) {
		AjaxJson j = new AjaxJson();
		try{
			maroPrinterService.addMain((MaroPrinterEntity)Util.jsonToObject(message,0),(List<MaroPrinterClassificationEntity>)Util.jsonToObject(message,1));
		}catch(Exception e){
			e.printStackTrace();
			throw new BusinessException(e.getMessage());
		}
	...
	public AjaxJson doUpdate(String message) {
		AjaxJson j = new AjaxJson();
		try {
			maroPrinterService.updateMain((MaroPrinterEntity)Util.jsonToObject(message,0),(List<MaroPrinterClassificationEntity>)Util.jsonToObject(message,1));
		} catch (Exception e) {
			e.printStackTrace();
			throw new BusinessException(e.getMessage());
		}
