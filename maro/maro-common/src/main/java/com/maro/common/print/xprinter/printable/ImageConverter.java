...
public class ImageConverter extends Converter<Image> {
	...
	public byte[] toBytes(DeviceSetting deviceSetting) {
		...
		if (0 == marginTop) {
			marginTop = WPConsts.DEFAULT_IMAGE_MARGIN;
		}
		if (0 == marginBottom) {
			marginBottom = WPConsts.DEFAULT_IMAGE_MARGIN;
		}
		...
		out.write(ESCPOSCode.ESC_FORWARD_DOT(marginTop));
		out.write(ESCPOSCode.IMAGE_PRINT((byte) 0x00, xL, xH, yL, yH, dest_bit_data)); // 该指令有打印功能
		out.write(ESCPOSCode.ESC_FORWARD_DOT(marginBottom));
