...
<div id="menu" style="mar">
            ...
            <ul>
                <li>
                   <input type="hidden" class="specificationsId" name="setmealDishesId" value="${ds.id}">
                    	...
                    	选 <input type="text" class="selectNum${ds.id}" name="selectNum" datatype="n" readonly onblur="ifSize(this,'${ds.id}')" style="width:50px;" value="${ds.selectNum!=null?ds.selectNum:1}"  onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}">
                    	</span>
                        	...
                        	<c:if test="${empty ds.setmealDishesSelectList}">
                              ...
                              <c:forEach items="${ds.setmealDishesSelectList}" var="dsSfc" varStatus="dsIndex">
                                <tr style="height: 42px;line-height: 42px;">
                        		  ...
                        		  <td><input id="consumptionQuantity" datatype="n" name="setmealDishesSelect[${dsIndex.index}].dishesNum" type="text" style="width: 100px" class="inputxt" value="${dsSfc.dishesNum}"/></td>
	...
	<tbody id="add_maroDishesSpecifications_table_template">
	   <tr style="height: 42px;line-height: 42px;">
	  		...
	  		<td><input id="consumptionQuantity" datatype="n" name="setmealDishesSelect[#index#].dishesNum" type="text" style="width: 100px" class="inputxt" value=""/></td>
	  		<td><input id="unit0" readonly="readonly" type="text" style="width: 100px;" class="inputxt unit#index#" value=""/></td>
	  		<td><a href="javascript:void(0)" class="ace_button" style="background-color:#ec4758;" onclick="delSelectTr(this)"><span class="sfc_del">删除(未保存)</span></a></td>
       </tr>
    </tbody>
	</table>
