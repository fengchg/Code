          ...
          <yd-scrolltab-panel v-for="item, index in menu" :key="index" :label="item.typeString" icon="">
            <yd-list theme="4" class="my-list-meal">
              <yd-list-item v-for="value, key in item.menuItemResultDTOList" :key="key">
                ...
                <div slot="title" class="my-list-title">
                  <span>{{value.maroDishesEntity.dishesName}}</span>
                  <span class="my-list-sales" v-salesStyle="{salesID:value.maroDishesEntity.id,saleList:salesList}">买 赠</span>
                </div>
                <div slot="title" class="my-list-title-note" >{{value.maroDishesEntity.describes}}</div>
                <yd-list-other slot="other">
                  <div>
                    <span class="my-price fc-theme" v-if="value.maroDishesEntity.type == 'package'"><em>¥</em>{{value.maroDishesEntity.packagePrice || '--'}}</span>
                    <span class="my-price fc-theme" v-else><em>¥</em>{{value.maroDishesEntity.salesPrice || '--'}}</span>
                  ...
                  <div v-if="value.maroDishesEntity.specificationsList">
                  ...
                  <div v-if="value.maroDishesEntity.setmealSishesList">
                    <template>
                      <button type="button" class="my-list-format-btn" @click="openFormatPopup(value.maroDishesEntity)">选套餐</button>
                    </template>
                    <!-- <cart-ctrl v-else :food="value.maroDishesEntity" :judge="true" @getFoodQuan="handleFoodQuan($event)" @zeroFoodQuan="zeroFoodQuan($event, index)"></cart-ctrl> -->
                    <!--<template v-else>-->
                      <!--<button type="button" class="my-list-format-btn" @click="openFormatPopup(value.maroDishesEntity)">+ </button>-->
...
Vue.directive('salesStyle',  function (el,binding) {
    let salesID = binding.value.salesID;
    let saleList = binding.value.saleList;
    for (let i = 0; i < saleList.length; i++) {
      if(saleList[i].buyid == salesID){
...
//      console.log(this.getSeatId);
      ...
      this.common.$ajax('/maroClientServerorderController.do?listMenu',param,function(data){
        ...
        that.handleMenu(data.obj.menuItemGroupResultDTOList);
          ...
          that.salesList = data.obj.salesPromotionList;
      })
...
//      console.log(arr);
      ...
      for(let i=0;i<arr.length;i++){
        if(!arr[i].type){
          this.$set(arr[i], 'type', -2)
          this.$set(arr[i], 'typeString',"套餐")
        }
         for(let j=0;j<arr[i].menuItemResultDTOList.length;j++){

           if (arr[i].menuItemResultDTOList[j].maroDishesEntity.setmealSishesList) {
             for (let n = 0; n < arr[i].menuItemResultDTOList[j].maroDishesEntity.setmealSishesList.length; n++) {
               this.$set(arr[i].menuItemResultDTOList[j].maroDishesEntity.setmealSishesList[n], 'selectList', [])
               let obj = arr[i].menuItemResultDTOList[j].maroDishesEntity.setmealSishesList[n]
               for (let k = 0; k < obj.setmealDishesSelectList.length; k++) {
                 Vue.set(obj.setmealDishesSelectList[k], 'isRepetitionhidde', false);
               }
             }
           }
...
//          console.log(this.currentFood);
          ...
          this.$set(this.pick[this.pick.length - 1], 'salesPrice', val.unitPrice)
          this.$set(this.pick[this.pick.length - 1], 'status', -1)
          this.$set(this.pick[this.pick.length - 1], 'packageType', 1)
...
//      console.log(this.getSeatId);
      ...
      this.$store.dispatch('setSales', this.salesList)
      this.$router.push({
        name: 'OrderConfirm'
      })
