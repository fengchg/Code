package com.maro.platform.jwt.util;
...
public class JwtHttpUtil {
	...
	public static JSONObject httpRequest(String requestUrl, String requestMethod, String outputStr, String sign) {
		...
		try {
			...
			e.printStackTrace();
			com.maro.platform.core.util.LogUtil.info("https request error:{}" + e.getMessage());
		} finally {
			try {
				httpUrlConn.disconnect();
			} catch (Exception e) {
				e.printStackTrace();
				com.maro.platform.core.util.LogUtil.info("http close error:{}" + e.getMessage());
			}
		}
		return jsonObject;
	}
