...
import com.maro.platform.core.online.util.FreemarkerHelper;
...
import com.maro.platform.tag.core.JeecgTag;
import com.maro.platform.tag.vo.easyui.ColumnValue;
import com.maro.platform.tag.vo.easyui.DataGridColumn;
import com.maro.platform.tag.vo.easyui.DataGridUrl;
import com.maro.platform.tag.vo.easyui.OptTypeDirection;
import com.maro.platform.web.cgform.common.CgAutoListConstant;
import com.maro.platform.web.cgform.entity.config.CgFormHeadEntity;
import com.maro.platform.web.cgform.entity.config.CgSubTableVO;
import com.maro.platform.web.cgform.service.config.CgFormFieldServiceI;
import com.maro.platform.web.cgform.util.PublicUtil;
				...
				if (col.isQuery()) {
					...
					if("single".equals(col.getQueryMode())){
						if(!StringUtil.isEmpty(col.getReplace())){
							...
							/*if(!StringUtil.isEmpty(col.getDictionary())){
...
//	                                    + " onMouseOver=\"toolTip('<img src='+value+'>')\" onMouseOut=\"toolTip()\" "
	...
	public String getNoAuthOperButton(){
		...
		//com.maro.platform.core.util.LogUtil.info("----getNoAuthOperButton-------"+sb.toString());
		return sb.toString(); 
	}
...
//		appendLine(sb,"<script type=\"text/javascript\" src=\"plug-in/jquery-plugs/storage/jquery.storageapi.min.js\" ></script>"); 
	...
	private void addSuperQuery(StringBuffer sb,String buttonSytle,List<DataGridColumn> columnList) {
		...
		String superQuery = free.parseTemplate("/com/maro/platform/tag/ftl/superquery.ftl", mainConfig);
	...
	public void getFilter(StringBuffer sb){
		if (this.isFilter()) {
			...
			String superQuery = free.parseTemplate("/com/maro/platform/tag/ftl/filter.ftl", mainConfig);
			appendLine(sb,superQuery);
		}
	}
	...
	private void addAdvancedQuery(StringBuffer sb,String buttonSytle) {
		...
		mainConfig.put("tableName", name);
		String complexSuperQuery = free.parseTemplate("/com/maro/platform/tag/ftl/complexSuperQuery.ftl", mainConfig);
	 ...
	 * 拼接数据字典里的条件
	 */
	private String p_dictionary(String[] dictionary){
		//判断 如果长度等于4，说明有筛选条件
		String dic = "";
