...
var shopId = '';
$('#test').on('change', '#sysCompanyCodeHtml', function() {
	shopId = $(this).find('option:selected').val();
	$("#coding").val('');
});

//编码失去焦点时
function checkCoding(th,au,userName){

	var sysCompanyCode = 0;
	if(userName == 'true'){ // 为子萱管理员
		var a = $("input[name='r']:checked").val();
	 	if(a == 0){
	 		sysCompanyCode = 0;
	 	}else if(a == 1){
	 		sysCompanyCode = shopId;
	 	}
 	}else if(userName == 'false'){
 		sysCompanyCode = $("#sysCompanyCode").val();
 	}
	...
	var coding = $(th).val();
	if(coding!=""){
		$.post("maroDishesController.do?checkCoding",{coding:coding,sysCompanyCode:sysCompanyCode},function(result){
...
function keyupSalesPrice(){alert(1);
	var salesPrice = $(".keyup").val();
	
	 var t01 = $("#add_maroDishesSpecifications_table tr").length;
	 alert(t01);
}
 ...
 $(document).ready(function () {
   $(".keyup").keyup(function(){
	  ...
	  var t01 = $("#add_maroDishesSpecifications_table tr").length;
	  if(t01 == 1){
	     $("input[name='maroDishesSpecificationsList[0].unitPrice']").val(salesPrice);
	  }
   });
});
