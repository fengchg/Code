...
public class WeixinMsgController extends MsgControllerAdapter {
    ...
    private MaroCommonMemberServiceI commonMemberService = SpringUtils.getBean(MaroCommonMemberServiceI.class);
    private MaroMemberServiceI memberService = SpringUtils.getBean(MaroMemberServiceI.class);
    ...
    protected void processInUserCardEvent(InUserCardEvent inUserCardEvent){
        String cardId = inUserCardEvent.getCardId();
        String code = inUserCardEvent.getUserCardCode();
        String event = inUserCardEvent.getEvent();
        String toUserName = inUserCardEvent.getToUserName();
        String fromUser = inUserCardEvent.getFromUserName();
        //获取会员信息
        ApiResult result = MemberCardApi.getUserInfo(cardId,code);
        //result.get("");
        if(InUserCardEvent.EVENT_MEMBERCARD.equals(event))
        {
            MaroCommonMemberEntity entity = commonMemberService.getByCode(code);
            if(entity==null){
                memberService.save(entity);
            }

        }
        //todo memberCommonService
    }
