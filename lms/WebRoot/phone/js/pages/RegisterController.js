...
BMSApp.pages.RegisterController = function () {
    //this.contactFunc = Template7.compile($$("#contactItemTpl").html());
};
BMSApp.pages.RegisterController.prototype.preprocess = function (content, url, next) {
    next(content);
};
BMSApp.pages.RegisterController.prototype.pageInit = function (page) {
    var self = this;
    //返回按钮注册  nav
    $$('.link.register-back').on('click', function () {
        myApp.loginScreen();
