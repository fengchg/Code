...
BMSApp.init = (function () {
    ...
    var preprocess = function (content, url, next) {
        ...
        if (!controller) {
            cb.loader.processNode(node, function () {
                controller = new BMSApp.pages[controllerName]();
    ...
    $$('#id_loginbtn').on('click',function(){
    	var username = $$('#id_usertxt').val();
    	var pass = $$('#id_passtxt').val();
//        $$.ajax({
//            url: 'http://127.0.0.1:8080/lms/phoneAction!login.action?user.userName='+username+'&user.password='+pass,
//            success:function(data){
//            	console.log(data);
//            }
//         });
    	$$.getJSON("phoneAction!login.action",{'user.userName':username,'user.password':pass},function(data){
    		var code = data.code;
    		if(code == 200){
    			var defaultOptions = {
    		            preprocess: function (content, url, next) {
...
//    			myApp.addView('.view-main', defaultOptions).router.loadPage('phoneAction!query.action');
    			myApp.addView('.view-main', defaultOptions).router.loadPage('pages/userList.html');
    			$$('#view-1').trigger('show');
    ...
    var loadLoginedAccount = function () {
        ...
        $$('#mylogin .list-login span.value-clear').on('click', function (e) {
            ...
            if ($$('#mylogin .list-login li').length == 0) {
                ...
                $$('#mylogin').find('span.value-clear.list-onOff').children('i').removeClass('icon-dropup').addClass('icon-dropdown');
            }
            e.stopPropagation();
        });
    ...
    $$('#mylogin').find('span.value-clear.list-onOff').on('click', function () {
    ...
    $$('.border-lightestgray-u.bg-green-u').on('click', function () {
        var defaultOptions = {
            preprocess: function (content, url, next) {
                return preprocess(content, url, next);
            }
